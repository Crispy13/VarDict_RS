...
Options:
  ...
  --java-parallel N    Java worker count (default: 10; use fewer for pileup to avoid OOM)
  --java-heap SIZE     Java heap size per worker (default: empty = no limit)
  ...
  --timeout SECONDS    Per-shard timeout for Java and Rust commands (default: 600)
    --retry N            Retry transient shard failures up to N times after compare (default: 0)
...
# --- gzip-aware helpers for compressed shard files (.tsv.gz) ---

gz_has_content() {
    [[ -f "$1" ]] && [[ -n "$(gzip -dc "$1" 2>/dev/null | head -c1)" ]]
}

gz_count_lines() {
    if [[ -f "$1" ]]; then
        gzip -dc "$1" | wc -l | tr -d '[:space:]'
    else
        printf '0'
    fi
}

gz_find_first_diff_line() {
    paste -d$'\x01' <(gzip -dc "$1") <(gzip -dc "$2") | awk -F$'\x01' '{ if ($1 != $2) { print NR; exit } }' || true
}
...
is_retryable_reason() {
    ...
    case "$reason" in
        timeout|java_command_failed|rust_command_failed|java_empty_suspect|rust_empty_suspect)
            return 0
            ;;
        *)
            return 1
...
needs_missing_outputs() {
    ...
    for ((idx = 0; idx < TOTAL_SHARDS; idx++)); do
        ...
        if [[ "$mode" == "java" ]]; then
            out_file="${JAVA_DIR}/shard_${SHARD_IDS[$idx]}.tsv.gz"
            if [[ ! -f "$out_file" ]]; then
        ...
        out_file="${RUST_DIR}/shard_${SHARD_IDS[$idx]}.tsv.gz"
...
generate_java_shard() {
    ...
    local java_file="${JAVA_DIR}/shard_${shard_num}.tsv.gz"
    ...
    local java_heap_flag=""
    [[ -n "$JAVA_HEAP" ]] && java_heap_flag="-Xmx${JAVA_HEAP}"
    # shellcheck disable=SC2086
    timeout "$SHARD_TIMEOUT" java $java_heap_flag -classpath "$JAVA_JAR" com.astrazeneca.vardict.Main \
        -G "$REF_FASTA" \
        -b "$BAM_PATH" \
        -N NA12878 $freq_flag -th 1 \
        $EXTRA_OPTS \
        -R "$region" \
        2>"$java_log" | gzip -1 >"$java_file" || java_rc=$?
...
generate_rust_shard() {
    ...
    local end=${SHARD_ENDS[$idx]}
    local region="${CHR}:${start}-${end}"
    local rust_file="${RUST_DIR}/shard_${shard_num}.tsv.gz"
    ...
    timeout "$SHARD_TIMEOUT" "$RUST_BIN" \
        ...
        $EXTRA_OPTS \
        -R "$region" \
        2>"$rust_log" | gzip -1 >"$rust_file" || rust_rc=$?
...
write_truncated_diff() {
    ...
    ( diff -u <(gzip -dc "$java_file") <(gzip -dc "$rust_file") || true ) | head -n 500 >"$diff_file"
}
...
compare_shard() {
    ...
    local java_has_content=0
    local rust_has_content=0
    gz_has_content "$java_file" && java_has_content=1
    gz_has_content "$rust_file" && rust_has_content=1

    if (( ! java_has_content && ! rust_has_content )); then
        clear_phase_failure "java" "$idx"
        clear_phase_failure "rust" "$idx"
        printf 'EMPTY\n' >"$status_file"
    ...
    if (( ! java_has_content && rust_has_content )); then
        rust_lines=$(gz_count_lines "$rust_file")
        printf 'FAIL\n' >"$status_file"
    ...
    if (( java_has_content && ! rust_has_content )); then
        java_lines=$(gz_count_lines "$java_file")
        printf 'FAIL\n' >"$status_file"
    ...
    if cmp -s <(gzip -dc "$java_file") <(gzip -dc "$rust_file"); then
        clear_phase_failure "java" "$idx"
        clear_phase_failure "rust" "$idx"
        printf 'PASS\n' >"$status_file"
        remove_stale_diff "$diff_file"
    ...
    java_lines=$(gz_count_lines "$java_file")
    rust_lines=$(gz_count_lines "$rust_file")
    first_diff_line=$(gz_find_first_diff_line "$java_file" "$rust_file")
...
collect_retryable_failure_indices() {
    local -n retry_indices_ref=$1
    local idx
    local status_file
    local status_value=""
    local meta_file
    ...
    for ((idx = 0; idx < TOTAL_SHARDS; idx++)); do
        ...
        # shellcheck disable=SC1090
        source "$meta_file"
        if is_retryable_reason "$REASON"; then
            retry_indices_ref+=("$idx")
        fi
    done
...
clear_retry_artifacts_for_idx() {
    local idx=$1
    local shard_num=${SHARD_IDS[$idx]}

    rm -f "$(status_file_for_idx "$idx")" "$(meta_file_for_idx "$idx")" "$(diff_file_for_idx "$idx")"
    remove_shard_verified "$idx"
    rm -f "$(phase_state_file_for_idx rust "$idx")" "${RUST_DIR}/shard_${shard_num}.tsv.gz" "${RUST_DIR}/shard_${shard_num}.log"

    if (( ! RUST_ONLY )); then
        rm -f "$(phase_state_file_for_idx java "$idx")" "${JAVA_DIR}/shard_${shard_num}.tsv.gz" "${JAVA_DIR}/shard_${shard_num}.log"
    fi
}
...
run_retry_attempts() {
    ...
    local idx
    local phase_label
    local -a retry_indices=()
    local -a retry_shards=()
    ...
    for ((attempt = 1; attempt <= MAX_RETRIES; attempt++)); do
        collect_retryable_failure_indices retry_indices || break

        retry_shards=()
        for idx in "${retry_indices[@]}"; do
            retry_shards+=("shard_${SHARD_IDS[$idx]}")
        done

        echo "--- RETRY attempt ${attempt}/${MAX_RETRIES}: retrying ${#retry_indices[@]} shards: ${retry_shards[*]} ---"
        phase_label="RETRY attempt ${attempt}/${MAX_RETRIES}"
        check_preflight_resources "$phase_label"

        for idx in "${retry_indices[@]}"; do
            clear_retry_artifacts_for_idx "$idx"
            if (( ! RUST_ONLY )); then
        ...
        tally_all_results
        echo "  RETRY attempt ${attempt}/${MAX_RETRIES} result: pass=${PASS_COUNT} fail=${FAIL_COUNT} empty=${EMPTY_COUNT}"
        ...
        for ((idx = 0; idx < TOTAL_SHARDS; idx++)); do
            ...
            if [[ -f "$status_file" ]]; then
                ...
                if [[ "$status_value" == "PASS" || "$status_value" == "EMPTY" ]]; then
                    shard_num=${SHARD_IDS[$idx]}
                    if (( CLEAN_ALL )); then
                        rm -f "${JAVA_DIR}/shard_${shard_num}.tsv.gz" "${JAVA_DIR}/shard_${shard_num}.log"
                    fi
                    rm -f "${RUST_DIR}/shard_${shard_num}.tsv.gz" "${RUST_DIR}/shard_${shard_num}.log"
...
run_parallel_phase() {
    ...
    local phase_parallel=${4:-$MAX_PARALLEL}
    ...
    for ((idx = 0; idx < phase_parallel; idx++)); do
        printf '\n' >&3
    done
...
run_chr() {
    ...
    if (( RUST_ONLY )); then
        ...
        echo "--- Phase 1: Java (${#java_needed_indices[@]} shards, ${JAVA_MAX_PARALLEL} parallel) ---"
        run_parallel_phase generate_java_shard "Phase 1 (Java)" java_needed_indices "$JAVA_MAX_PARALLEL" || fail "Phase 1 (Java) aborted due to low memory"
    else
    ...
    if (( ${#rust_needed_indices[@]} > 0 )); then
        ...
        run_parallel_phase generate_rust_shard "Phase 2 (Rust)" rust_needed_indices "$MAX_PARALLEL" || fail "Phase 2 (Rust) aborted due to low memory"
    else
...
main() {
    ...
    validate_nonnegative_integer "retry count" "$MAX_RETRIES"
