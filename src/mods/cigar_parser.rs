...
use crate::{
    ...
    prelude::{InnerMap, LibDefaultHasher, SmallVecBytes},
...
pub struct CigarParser {
    ...
    non_insertion_vars: HashMap<i64, InnerMap<VarDesc, Variant>, LibDefaultHasher>,
    non_insertion_vars_insert_index: HashMap<i64, usize, LibDefaultHasher>,
    next_non_insertion_vars_insert_index: usize,
    insertion_vars: HashMap<i64, InnerMap<VarDesc, Variant>, LibDefaultHasher>,

    /// Track MNPs (multi-nucleotide polymorphisms) by position and description string
    mnp: HashMap<i64, InnerMap<String, usize>, LibDefaultHasher>,

    /// Track insertion counts by position and description string (Java: positionToInsertionCount)
    position_to_insertion_count: HashMap<i64, InnerMap<String, usize>, LibDefaultHasher>,
    ...
    svffus: HashMap<i32, Vec<SoftClip>, LibDefaultHasher>,
    svrfus: HashMap<i32, Vec<SoftClip>, LibDefaultHasher>,
    svfusfend: HashMap<i32, i64, LibDefaultHasher>,
    svfusrend: HashMap<i32, i64, LibDefaultHasher>,
...
impl Default for CigarParser {
    fn default() -> Self {
        Self {
            ...
            svffus: Default::default(),
            svrfus: Default::default(),
            svfusfend: Default::default(),
            svfusrend: Default::default(),
...
impl CigarParser {
    ...
    pub fn get_non_insertion_vars(
        &self,
    ) -> &HashMap<i64, InnerMap<VarDesc, Variant>, LibDefaultHasher> {
    ...
    pub fn take_non_insertion_vars_insert_index(
        &mut self,
    ) -> HashMap<i64, usize, LibDefaultHasher> {
    ...
    pub fn take_svffus(&mut self) -> HashMap<i32, Vec<SoftClip>, LibDefaultHasher> {
        std::mem::take(&mut self.svffus)
    }

    pub fn take_svrfus(&mut self) -> HashMap<i32, Vec<SoftClip>, LibDefaultHasher> {
        std::mem::take(&mut self.svrfus)
    }
        ...
        if !skip_mate_sv_assignment {
            ...
            if read_dir_num == 1 {
                let should_start_new = self
                    .svfusfend
                    .get(&mate_tid)
                    .map(|last_end| (start - *last_end) as f64 > min_cluster_dist)
                    .unwrap_or(true);
                let bucket = self.svffus.entry(mate_tid).or_default();
                if should_start_new || bucket.is_empty() {
                    bucket.push(SoftClip::default());
                }
                ...
                self.svfusfend.insert(mate_tid, end);
            } else {
                let should_start_new = self
                    .svfusrend
                    .get(&mate_tid)
                    .map(|last_end| (start - *last_end) as f64 > min_cluster_dist)
                    .unwrap_or(true);
                let bucket = self.svrfus.entry(mate_tid).or_default();
                if should_start_new || bucket.is_empty() {
                    bucket.push(SoftClip::default());
                }
                if let Some(last) = bucket.last_mut() {
    ...
    fn parse_cigar(&mut self, record: &mut Record) -> Result<(), Error> {
        ...
        query_seq_owned.clear();
        query_seq_owned.extend(record.seq().into_decoded_base_iter());

        let mut query_qual_owned = self
            .query_qual_buf
            .take()
            .unwrap_or_else(|| Vec::with_capacity(512));
        query_qual_owned.clear();
        query_qual_owned.extend_from_slice(record.qual());
        ...
        let result = (|| -> Result<(), Error> {
            ...
            let tot_nm = match record.aux_option(self.aligner.nm_tag())? {
                ...
                None => {
                    ...
                    // Java: skip unmapped reads (issue #56) and reads without alignment
            ...
            if self.instance.conf.perform_local_realignment {
                ...
                if local_pos < 0 {
                    ...
                    let modified_cigar = mc.cigar.iter().copied().collect::<Vec<_>>();
                    cigar = CigarString(modified_cigar).into_view(pos);
            ...
            'process_cigar: {
                ...
                while ci < self.cigar.len() {
                    ...
                    if trace_target {
                        event!(
                            Level::WARN,
                            "[trace_target] qname={} ci={} op={:?} len={} start={} read_pos_incl={} read_pos_excl={}",
                    ...
                    while i < self.cigar_len as usize {
                        ...
                        let mut q =
                            *query_qual.get_or_err(self.read_pos_including_softclip)? as f64;
                        let mut qbases = 1;
                        let mut qibases = 0;
                        ...
                        loop {
                            ...
                            let nuc =
                                *query_seq.get_or_err(self.read_pos_including_softclip + 1)?;
                            if nuc == b'N' {
                                break;
                            }
                            ...
                            match self.reference.get(self.start + 1) {
                                Some(ref_base) => {
                                    if nuc != ref_base {
                                        ...
                                        for ssi in 1..=ssn {
                                            ss.push(*query_seq.get_or_err(
                                                self.read_pos_including_softclip + ssi,
                                            )?);
                                            q += (*query_qual.get_or_err(
                                                self.read_pos_including_softclip + ssi,
                                            )?)
                                                as f64;
                                            qbases += 1;
        ...
        self.query_seq_buf = Some(query_seq_owned);
        self.query_qual_buf = Some(query_qual_owned);
    ...
    fn process_soft_clip(
        ...
        if ci == 0 {
            ...
            */
            ...
            // Java's 3' recapture loop does NOT check cigarElementLength as a
            // termination condition — it continues as long as read bases match
            // reference, even beyond the nominal soft clip length. This allows
            // recovery of bases skipped by stale offset in prior M-blocks.
                ...
                && *query_quality.get_or_err(self.read_pos_including_softclip)? > 10
            {
                ...
                self.read_pos_excluding_softclip += 1;
                self.start += 1;
                cigar_len = cigar_len.saturating_sub(1);
            }
            ...
            let remaining_softclip_len = (cigar_len as usize).min(
                query_sequence
                    .len()
                    .saturating_sub(self.read_pos_including_softclip),
            );

            if remaining_softclip_len > 0 {
                let mut read_qual_sum = 0;
                let mut num_high_qual_base = 0;
                let mut num_low_qual_base = 0;
                for si in 0..remaining_softclip_len {
                    // Loop over remaining soft-clipped sequence
                    // Stop if unknown base (N - any of ATGC) is found

                    // At this point, self.read_pos_including_softclip is start offset of soft clip
                    if query_sequence
                        .get_or_err(self.read_pos_including_softclip + si)
                        .copied()?
                        == b'N'
                    {
                        break;
    ...
    fn process_deletion(
        ...
        if is_followed_by_match_and_indel(&self.cigar, ci) {
            ...
            if is_next_after_num_matched(&self.cigar, ci, 3) {
                ...
                let ts = self.start + ref_offset as i64 + self.cigar_len as i64;
            ...
            if is_next_after_num_matched(&self.cigar, ci, 2) {
                ...
                let tn = self.read_pos_including_softclip + read_offset;
                let ts = self.start + self.cigar_len as i64;
                let (offset, nmoff_delta) = scan_offset_extension(
                    &self.reference,
                    ts,
                    tn,
            ...
            let tn = self.read_pos_including_softclip;
            let ts = self.start + self.cigar_len as i64;
            let (offset, nmoff_delta) = scan_offset_extension(
                &self.reference,
                ts,
    ...
    fn find_offset(
        ...
        let (offset, nmoff) = scan_offset_extension(
            &self.reference,
            ts as i64,
            tn,
            seg_len,
    ...
    fn add_variation_for_matching_part(
        ...
        if did_add_variant {
            let shift = if is_insertion && has_ampersand { 1 } else { 0 };
    ...
    fn increment_position_count(
        map: &mut HashMap<i64, InnerMap<String, usize>, LibDefaultHasher>,
        pos: i64,
        key: &str,
    ) {
...
fn format_variation_pos_map(
    map: &HashMap<i64, InnerMap<VarDesc, Variant>, LibDefaultHasher>,
) -> String {
...
fn scan_offset_extension(
    ...
    while vsn <= vext && vi < seg_len {
        let seq_base = *query_seq.get_or_err(tn + vi)?;
        if seq_base == b'N' {
            break;
        }

        let qual = *query_qual.get_or_err(tn + vi)? as f64;
        if qual < goodq {
            break;
        }

        match reference.get(ts + vi as i64) {
            Some(ref_base) => {
                if stop_on_ref_n && ref_base == b'N' {
                    break;
                }
...
mod tests {
    ...
    #[test]
    fn test_scan_offset_extension_reports_last_mismatch_and_nmoff() {
        let reference = Reference::new_with_start(b"ACGT".to_vec(), 1);
        let query_seq = b"ATGA";
        let query_qual = [30u8; 4];

        let result = scan_offset_extension(
            &reference,
            1,
            0,
            query_seq.len(),
    ...
    #[test]
    fn test_scan_offset_extension_can_stop_on_reference_n() {
        let reference = Reference::new_with_start(b"ACNT".to_vec(), 1);
        let query_seq = b"ACGT";
        let query_qual = [30u8; 4];

        let stop_on_ref_n = scan_offset_extension(
            &reference,
            1,
            0,
            query_seq.len(),
        ...
        .expect("scan should succeed");
        let allow_ref_n_mismatch = scan_offset_extension(
            &reference,
            1,
            0,
            query_seq.len(),
